...
`}function y(){return`
    ...
    `}let o,u=null;document.addEventListener("DOMContentLoaded",()=>{o=new h("#iepmc-table",!1,!0,!1,!0,"./api/iepmc/page"),p.CreateIEPMCValidation(l=>{l.preventDefault();let a=b.serializeArrayToJson(".maintenance_form");for(let e="a".charCodeAt(0);e<="z".charCodeAt(0);e++){let i=String.fromCharCode(e),t=$('input[name="'+i+'"]');if(i=="a"||i=="b")for(let d=1;d<=9;d++){let c=$('input[name="'+i+d+'"]');console.log(i+d),c.is(":checked")?a[i+d]="✔":a[i+d]=""}t.is(":checked")?a[i]="✔":a[i]=""}r(a,"./iepmc/stream-template",e=>{console.log("Serialize",a),v.create(a,e,()=>{o.table.ajax.reload(),$("#maintenanceModal").modal("hide")})})}),p.UpdateIEPMCValidation(l=>{l.preventDefault();let a=b.serializeArrayToJson(".update-maintenance_form");for(let e="a".charCodeAt(0);e<="z".charCodeAt(0);e++){let i=String.fromCharCode(e),t=$('input[name="'+i+'"]');if(i=="a"||i=="b")for(let d=1;d<=9;d++){let c=$('input[name="'+i+d+'"]');console.log(i+d),c.is(":checked")?a[i+d]="✔":a[i+d]=""}t.is(":checked")?a[i]="✔":a[i]=""}r(a,"./iepmc/stream-template",e=>{a.id=u.id,v.update(a,e,()=>{o.table.ajax.reload(),$("#update-maintenanceModal").modal("hide")})})}),o.custom_buttons=l=>{let a=document.createElement("div");return n.data=l,n.viewName="View",n.viewAction=()=>{window.open("iepmc/office/stream?path=iepmc/stream/2025/"+l.id)},n.deleteAction=e=>{m.load(()=>{e.target.dataset.id,o.table.ajax.reload()},"Do you want to delete this file ?")},n.updateAction=e=>{v.fetch_item(l.id,i=>{u=i,document.getElementById("update-maintenance_form").innerHTML=x(i),document.getElementById("maintenance_form").innerHTML="",$("#update-maintenanceModal").modal("show")})},n.loadButtons(a),a},o.load(["document_number","property_number","issued_to","brand_model","mac_address","inspected_by"]),$("#create_iepmc_btn").on("click",function(){document.getElementById("maintenance_form").innerHTML=y(),$("#maintenanceModal").modal("show")}),$(".submit_iepmc_btn").on("click",function(){$("#maintenance_form").trigger("submit")}),$(".submit_update_iepmc_btn").on("click",function(){$("#update-maintenance_form").trigger("submit")})});
