...
<div class="ui top attached segment">
    <div class="ui two column grid">
        <div class="column">
            <button class="ui grey very tiny button">SUPPLEMENTAL UPDATE FORM</button>
        </div>
        <div class="column" style="text-align: right">
            <button class="ui green very tiny button submit_supplemental_form_button">SAVE CHANGES</button>
        </div>
    </div>
</div>
    ...
    <form class="ui form formCreateSupplemental" action="#" id="formCreateSupplemental" method="post">
        ...
        <div class="field">
            <div  class="two fields">
                <div class="field">
                    <label>TITLE</label>
                    <input type="text" name="title" value="{{ $supplemental->title }}" placeholder="TITLE" value="{{ date('Y-m')." SUPPLEMENTAL" }}">
                </div>
...
@section('custom_js')
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            localStorage.setItem('supplemental_id', "{{ $supplemental->id }}");
        })
    </script>
    @vite(['resources/js/supplemental/update.js'])
@endsection